  ...
  return new Promise((resolve, reject) => {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        var list = result.data.song.list;
        for (var i = list.length-1; i>=0; i--) {
          // 如果这首歌需要付费播放，则将这首歌从列表中去除
          if (list[i].pay.payplay == 1) {
            list.splice(i,1)
            continue
          }
          ...
          if (list[i].albumid == 0) {
            ...
            list[i].albumImg = 'http://imgcache.qq.com/music/photo/album_300/' + list[i].albumid % 100 + '/300_albumpic_' +           list[i].albumid + '_0.jpg'
          }
        }
        resolve(list)
      },
